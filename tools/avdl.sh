...
if  [ -z "$1" ]; then
    ...
    # CS: 24 Oct 2024 13:27 the above arg doesn't work for xiaohongshu
    echo $1 | grep -q xiaohongshu && CS_VID_QUALITY="bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best"
fi
...
if [ "$cho" == "n" ]; then
	echo -n -e  "Media type ${GREEN}(a)${NORMAL}udio or ${GREEN}(v)${NORMAL}ideo, or just play the stream video using iina: "

    echo

    cho=$(gum choose "audio" "video" "play online")
...
case $cho in
    ...
    # CS: 20 Oct 2024 18:52
    "play online" | "play" | "p" | 3)
        iina "$url"
    ;;
